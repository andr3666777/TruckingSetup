...
export default function Hero() {
  return (
    <section id="home" className="relative min-h-screen flex items-center overflow-hidden pt-28 pb-20 lg:pb-16">
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-navy-950/40 via-ink-950 to-ink-950" />
      <div className="absolute inset-0 hero-grid" />
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-radial-glow" />

      {/* Floating accent orbs */}
      <div className="absolute top-1/4 -left-20 w-72 h-72 bg-navy-600/20 rounded-full blur-[100px] animate-pulse-slow" />
      <div className="absolute bottom-1/4 -right-20 w-80 h-80 bg-accent-500/10 rounded-full blur-[120px] animate-pulse-slow" />
        ...
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-7 text-center lg:text-left">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass mb-6 animate-fade-down">
              ...
              <span className="text-xs font-medium text-ink-200">Trusted by 500+ trucking entrepreneurs</span>
            </div>
            ...
            {/* Subheadline */}
            <p className="text-base sm:text-lg text-ink-300 max-w-2xl mx-auto lg:mx-0 mb-8 leading-relaxed animate-fade-up [animation-delay:0.15s] opacity-0">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-2xl mx-auto lg:mx-0 animate-fade-up [animation-delay:0.45s] opacity-0">
              {trustBadges.map((badge) => (
                ...
                >
                  ...
                  <span className="text-xs font-medium text-ink-200 leading-tight">{badge.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative animate-scale-in [animation-delay:0.2s] opacity-0 mt-8 lg:mt-0">
            <div className="relative max-w-sm mx-auto lg:max-w-none">
              ...
              {/* Floating stat card — hidden on small screens to prevent overflow */}
              <div className="hidden sm:flex absolute -bottom-6 -left-6 glass-dark rounded-2xl p-4 shadow-2xl animate-float items-center gap-3">
                ...
                <div>
                  <div className="font-display font-bold text-2xl text-white">95%</div>
                  <div className="text-xs text-ink-300">Client Satisfaction</div>
                </div>
              </div>

              {/* Floating badge — hidden on small screens to prevent overflow */}
              <div className="hidden sm:flex absolute -top-4 -right-4 glass-dark rounded-2xl px-4 py-3 shadow-2xl animate-float [animation-delay:1s] items-center gap-2">
                ...
                <span className="text-sm font-semibold text-white">MC & DOT Filed</span>
              </div>
            </div>
          </div>
        </div>
