...
function StatCard({ stat, start }: { stat: (typeof stats)[number]; start: boolean }) {
  ...
  return (
    <div className="glass-card p-6 lg:p-8 text-center group">
      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-navy-600/30 dark:to-navy-800/30 border border-slate-200 dark:border-navy-500/20 flex items-center justify-center mx-auto mb-5 group-hover:from-accent-500 group-hover:to-accent-600 group-hover:border-accent-400/30 transition-all duration-500 group-hover:scale-110">
        <stat.icon className="w-7 h-7 text-accent-500 dark:text-navy-300 group-hover:text-white transition-colors duration-500" />
      </div>
      <div className="font-display font-extrabold text-3xl sm:text-4xl lg:text-5xl text-slate-800 dark:text-white mb-2">
        {stat.isText ? (
      ...
      <div className="text-sm text-slate-500 dark:text-ink-300 font-medium">{stat.label}</div>
    </div>
  );
}
...
export default function WhyChooseUs() {
  ...
  return (
    <section id="why-us" className="relative py-24 lg:py-32 overflow-hidden bg-slate-100/50 dark:bg-ink-900/30">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-navy-400/10 dark:bg-navy-600/10 rounded-full blur-[120px]" />
      ...
      <div className="relative section-pad max-w-7xl mx-auto">
        <div ref={ref} className={`reveal ${isVisible ? 'is-visible' : ''} text-center max-w-3xl mx-auto mb-16`}>
          ...
          <p className="text-slate-600 dark:text-ink-300 text-lg max-w-2xl mx-auto">
